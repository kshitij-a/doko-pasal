...
export default function MobileBottomBar({ cartCount = 0, cartTotal = 0, onCartOpen }: MobileBottomBarProps) {
  ...
  const [local, setLocal] = useState({ count: 0, total: 0 })
  useEffect(() => {
    const read = () => {
      try {
        const c = JSON.parse(localStorage.getItem('cart') || '[]')
        setLocal({
          count: c.reduce((a: any, i: any) => a + (i.qty || 0), 0),
          total: c.reduce((a: any, i: any) => a + (i.price || 0) * (i.qty || 0), 0),
        })
      } catch { /* corrupt cart — badge stays 0 */ }
    }
    read()
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[109] sm:hidden bg-white border-t border-[#E8E3DB] safe-area-bottom">
        <div className="flex items-center justify-around px-1 py-1.5">
          {items.map((item, i) => {
            if (item.type === 'cart') {
              return (
                <button key={i} onClick={handleCartOpen}
