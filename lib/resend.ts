...
type OrderItem = { product_name: string; size?: string; quantity: number; price: number }
...
export async function sendOrderEmail({
  ...
  const bankName = bank?.name || process.env.BANK_NAME || 'Nepal Investment Bank'
  const bankAccountName = bank?.accountName || process.env.BANK_ACCOUNT_NAME || 'Doko Pasal'
  const bankAccountNo = bank?.accountNo || process.env.BANK_ACCOUNT_NO || '001234567890'
