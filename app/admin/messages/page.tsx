                  ...
                  <button key={conv.id} onClick={() => selectConversation(conv)}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-0.5">
                        <p className={`text-sm truncate ${conv.unread_count > 0 ? 'font-extrabold text-white' : 'font-bold text-gray-300'}`}>{conv.user_name}</p>
                        <p className="text-gray-500 text-xs flex-shrink-0 ml-2">{timeAgo(conv.last_message_at)}</p>
                      </div>
                      <p className={`text-xs truncate ${conv.unread_count > 0 ? 'text-gray-300 font-semibold' : 'text-gray-400'}`}>{conv.last_message || 'No messages yet'}</p>
